...
Acceleration AccelerationFunction(StartingData Data, State State, double AirResistance){
    ...
    Acc.Acceleration1 = a1pure - (AirResistance * State.Velocity1 / Data.Mass1);
    ...
    Acc.Acceleration2 = a2pure - (AirResistance * State.Velocity2 / Data.Mass2);
...
State UpdateState(StartingData Data, State state, double Time, double AirResistance){
    ...
    Acceleration Acc1 = AccelerationFunction(Data, current, AirResistance);
    ...
    Acceleration Acc2 = AccelerationFunction(Data, s1, AirResistance);
    ...
    Acceleration Acc3 = AccelerationFunction(Data, s2, AirResistance);
    ...
    Acceleration Acc4 = AccelerationFunction(Data, s3, AirResistance);
    ...
    while (current.Angle1 > PI) current.Angle1 -= 2.0 * PI;
    while (current.Angle1 < -PI) current.Angle1 += 2.0 * PI;

    while (current.Angle2 > PI) current.Angle2 -= 2.0 * PI;
    while (current.Angle2 < -PI) current.Angle2 += 2.0 * PI;
